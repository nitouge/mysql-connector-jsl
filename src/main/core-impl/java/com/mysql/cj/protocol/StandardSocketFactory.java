...
public class StandardSocketFactory implements SocketFactory {
                ...
                for (int i = 0; i < possibleAddresses.length; i++) {
                    try {
                        ...
                        if (localSockAddr != null) {
                            ...
                            System.out.println("StandardSocketFactory bind: " + localSockAddr);
                        }

                        this.rawSocket.connect(sockAddr, getRealTimeout(connectTimeout));
                        System.out.println("StandardSocketFactory Socket connect and timeout: " + connectTimeout);
                        break;
    ...
    public void beforeHandshake() throws IOException {
        System.out.println("\n------------StandardSocketFactory beforeHandshake");
        resetLoginTimeCountdown();
        this.socketTimeoutBackup = this.rawSocket.getSoTimeout();
        int realTimeout = getRealTimeout(this.socketTimeoutBackup);
        System.out.println("StandardSocketFactory realTimeout: " + realTimeout + ", and socketTimeoutBackup: " + this.socketTimeoutBackup);
        this.rawSocket.setSoTimeout(realTimeout);
        System.out.println(">>> StandardSocketFactory beforeHandshake end and return socket: " + this.rawSocket);
    }
    ...
    protected void resetLoginTimeCountdown() throws SocketException {
        if (this.loginTimeoutCountdown > 0) {
            ...
            if (this.loginTimeoutCountdown <= 0) {
                System.out.println("*********** StandardSocketFactory loginTimeoutCountdown: " + this.loginTimeoutCountdown + ", and socketTimeoutBackup: " + this.socketTimeoutBackup);
