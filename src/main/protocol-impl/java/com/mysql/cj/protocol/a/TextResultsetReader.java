...
import com.mysql.cj.protocol.a.result.CircularBuffer;
import com.mysql.cj.protocol.a.result.OkPacket;
import com.mysql.cj.protocol.a.result.ResultsetCircularBufferRowsConsuming;
import com.mysql.cj.protocol.a.result.ResultsetRowsConsuming;
...
public class TextResultsetReader implements ProtocolEntityReader<Resultset, NativePacketPayload> {
    ...
    private final boolean forwardConsuming;

    public TextResultsetReader(NativeProtocol prot) {
        this.protocol = prot;
        // 安全获取属性（避免NPE）
        this.forwardConsuming = this.protocol.getPropertySet()
                .getBooleanProperty(PropertyKey.forwardConsuming)
                .getValue();
    }
    ...
    public Resultset read(int maxRows, boolean streamResults, NativePacketPayload resultPacket, ColumnDefinition metadata,
        ...
        if (columnCount > 0) {
            ...
            if (!streamResults) {
                ...
                if (!this.protocol.isInternalQuery() && forwardConsuming
                        && resultSetFactory.getResultSetType()== Resultset.Type.FORWARD_ONLY
                        && resultSetFactory.getResultSetConcurrency() == Resultset.Concurrency.READ_ONLY) {

                    System.out.println("\n##################### 使用自定义forwardConsuming存储结果集 #####################");

                    // LinkedList<ResultsetRow> rowList = new LinkedList<>();
                    // ResultsetRow row;
                    // while ((row = this.protocol.read(ResultsetRow.class, trf)) != null) {
                    //     if (maxRows == -1 || rowList.size() < maxRows) {
                    //         rowList.add(row);
                    //     } else {
                    //         // 超过maxRows时继续读取但不存储（避免协议中断）
                    //         continue;
                    //     }
                    // }
                    // rows = new ResultsetRowsConsuming(rowList, cdef);

                    // 使用用环形缓冲
                    CircularBuffer<ResultsetRow> buffer = new CircularBuffer<>(1000); // 初始容量
                    ResultsetRow row = this.protocol.read(ResultsetRow.class, trf);
                    while (row != null) {
                        if (maxRows == -1 || buffer.size() < maxRows) {
                            buffer.add(row);
                        }
                    ...
                    // 创建消费式结果集包装
                    rows = new ResultsetCircularBufferRowsConsuming(buffer, cdef);

                    System.out.println("##################### 使用自定义forwardConsuming存储结果集完成 #####################\n");
                } else {
